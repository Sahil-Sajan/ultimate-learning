      ...
      <div className="max-w-360 mx-auto flex items-center justify-between px-6 py-4">
        ...
        <Link href="/" className="flex items-center gap-3 shrink-0 group z-110" onClick={closeAllMenus}>
          ...
          <div className="flex flex-col leading-none">
            <span className="text-xl font-bold uppercase tracking-tight">Ultimate</span>
            <span className="text-xs font-light text-[#FF5B5C] uppercase tracking-[0.2em]">Learning</span>
          </div>
        </Link>
          ...
          {navLinks.map((link) => (
            ...
            >
              ...
              >
                {link.name}
                {link.megaType && (
                  <ChevronDown size={14} className={`transition-transform duration-300 ${activeMenu === link.megaType ? "rotate-180" : ""}`} />
                )}
              </Link>
                        ...
                        <div className="col-span-3 border-r border-white/5 pr-6">
                          ...
                          <div className="space-y-0.5">
                            {link.megaType === "courses" ? (
                              <>
                                ...
                                <MegaSidebarItem onClick={closeAllMenus} href="/courses" icon={<Smartphone size={18} />} label="Mobile Science" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/courses" icon={<Database size={18} />} label="Data Science" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/courses" icon={<Palette size={18} />} label="Digital Design" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/courses" icon={<Briefcase size={18} />} label="Business" />
                              </>
                            ) : (
                              <>
                                <MegaSidebarItem onClick={closeAllMenus} href="/instructors" icon={<Users size={18} />} label="Senior Mentors" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/instructors" icon={<Zap size={18} />} label="Industry Leaders" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/instructors" icon={<UserCheck size={18} />} label="Vetted Tutors" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/instructors" icon={<Briefcase size={18} />} label="Corporate Coaches" />
                                <MegaSidebarItem onClick={closeAllMenus} href="/instructors" icon={<Monitor size={18} />} label="Tech Architects" />
                              </>
                            )}
                          </div>
                          <Link href={link.href} onClick={closeAllMenus}>
                            <button className="w-full mt-8 bg-[#FF5B5C] py-4 rounded-2xl font-bold text-sm hover:bg-[#ff4646] transition-all flex items-center justify-center gap-2">
                              {link.megaType === "courses" ? "View All Courses" : "Meet All Instructors"} <ArrowRight size={16} />
                            </button>
                          </Link>
                        </div>
              ...
              <div key={link.name} className="border-b border-white/10 pb-4">
                ...
                {link.megaType && mobileAccordion === link.megaType && (
                  <div className="mt-4 grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-4">
                    {(link.megaType === "courses" 
                      ? ["Web Design", "Cloud Computing", "AI Content", "Cyber Security"] 
                      : ["Meet Mentors", "Teacher FAQ", "Become a Tutor", "Live Help"]
                    ).map((item) => (
                      <Link key={item} href={link.href} onClick={closeAllMenus} className="bg-white/5 p-4 rounded-2xl text-sm font-semibold text-white/70 active:bg-[#FF5B5C] active:text-white transition-colors">
                        {item}
                      </Link>
                    ))}
