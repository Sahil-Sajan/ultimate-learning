...
interface CourseCardProps {
  title: string;
  image?: string; // Image prop add ki gayi hai
  students: number;
  lastActive: string;
...
export default function MyCoursesPage() {
  return (
    <div className="space-y-8 pb-10">
      ...
      {/* 3. Courses I'm Teaching Section (IMAGES ADDED HERE) */}
...
function CourseCard({
  ...
  return (
    <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex flex-col hover:border-rose-200 transition-all group relative overflow-hidden">
      {isNew && (
        <span className="absolute top-4 right-4 bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-lg shadow-sm z-10">
          NEW!
        </span>
      )}

      {/* Course Image Section (Replaced Placeholder) */}
