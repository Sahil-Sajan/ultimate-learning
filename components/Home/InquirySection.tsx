            ...
            {[
              {
                ...
                bg: "bg-purple-100",
                desc: "Learning from anywhere has become a transform aspect of modern education.",
              },
              {
                icon: <Award className="w-6 h-6 text-pink-500" />,
                title: "Expert Mentors",
                bg: "bg-pink-100",
                desc: "Expert mentors are invaluable assets in any field, providing seasoned guidance.",
              },
              {
                icon: <Zap className="w-6 h-6 text-cyan-500" />,
                title: "Learn in demand skills",
                bg: "bg-cyan-100",
                desc: "In today's rapidly evolving job market, learning skills is crucial.",
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
              ...
              >
                <div
                  className={`shrink-0 w-12 h-12 ${feature.bg} rounded-full flex items-center justify-center`}
                >
                  {feature.icon}
                </div>
                <div>
          ...
          >
            <div className="flex -space-x-2 lg:-space-x-3">
              {avatars.map((url, i) => (
                <div
                  key={i}
                  className="w-8 h-8 lg:w-10 lg:h-10 rounded-full border-2 border-white overflow-hidden relative"
                >
                  <Image
                    src={url}
                    alt="User"
